...
Modified by JoeBlakeB to take session cookie from argument
and for file structure to not have day folders
...
IMAGE_DIR = AOC_DIR / ".github/AoCTiles/images"
...
def get_solution_paths_dict_for_years() -> dict[int, dict[int, list[str]]]:
    ...
    E.g.: {2022: {1: [Path("2022/01.py"), Path("2022/01.kt")], ...}}
    ...
    for year_dir in sorted(get_paths_matching_regex(AOC_DIR, YEAR_PATTERN), reverse=True):
        ...
        year_dict = {a: [] for a in range(1, 26)}
        print(year_dir)

        solutions = sorted(find_recursive_solution_files(year_dir))
        ...
        solution_paths_dict[year] = year_dict
...
main_font = functools.partial(get_font, path=AOC_TILES_SCRIPT_DIR / "PaytoneOne.ttf")
secondary_font = functools.partial(get_font, path=AOC_TILES_SCRIPT_DIR / "SourceCodePro-Regular.otf")
...
def handle_day(day: int, year: int, solutions: list[str], html: HTML, day_scores: DayScores | None, needs_update: bool):
    ...
    with html.tag("a", href=str(solution_link)):
        html.tag("img", closing=False, src=day_graphic_path.as_posix(), width=TILE_WIDTH_PX, style="style='min-width: calc(20 % - 4px);'")
...
def main():
    ...
    for year, day_to_solutions_list in get_solution_paths_dict_for_years().items():
        ...
        totalStars += handle_year(year, day_to_solutions_list)

    os.system(r"sed -Ei 's/(https:\/\/img\.shields\.io\/badge\/Total_Stars-)[0-9]+(_⭐-blue)/https:\/\/img\.shields\.io\/badge\/Total_Stars-" + str(totalStars) + "_⭐-blue/g' README.md")
